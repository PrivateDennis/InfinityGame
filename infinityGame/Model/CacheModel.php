...
use Phpfastcache\Drivers\Files\Item;
...
class CacheModel
{
    ...
    public static function getHashFromKey(string $key){
        return self::getInstanceCache()->getConfig()->getDefaultFileNameHashFunction()($key);
    }

    public static function kill(string $key)
    {
        self::$trace[] = 'deleting '.$key.' hash: '.self::getHashFromKey($key);

        $instance = self::getInstanceCache();
        if ($instance->hasItem($key)) {
    ...
    {
        return self::getInstanceCache()->getItem($key);
    }

    public static function save(CacheItemInterface $item)
    {
        return self::getInstanceCache()->save($item);
    }
...
//                self::$trace[] = 'Skip key: ' . $i['key'] . ' expired:' . $i['expireTime'] . ' size:' . $i['filesize'];
            ...
            if (unlink($i['filename'])) {
                self::$trace[] = 'removed expired key: ' . $i['key'] . ' expired:' . $i['expireTime'] . ' size:' . $i['filesize'];
            } else {
        ...
        self::$trace[] = 'removed ' . $sumDeleted . ' files with combined size: ' . $sumSize;
